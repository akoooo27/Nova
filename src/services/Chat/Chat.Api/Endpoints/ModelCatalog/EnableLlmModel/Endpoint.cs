...
namespace Chat.Api.Endpoints.ModelCatalog.EnableLlmModel;

internal sealed class Endpoint(ISender sender) : EndpointWithoutRequest<LlmModelResponse>
{
    public const string RouteName = "Chat.ModelCatalog.LlmModels.Enable";

    public override void Configure()
    {
        Put("/model-catalog/providers/{providerId}/models/{modelId}/enable");
        Version(1);
        ...
        {
            descriptor
                .WithSummary("Enable LLM Model")
                .WithDescription("Re-enables an existing LLM model in the model catalog.")
    ...
    {
        EnableLlmModelCommand command = new
        (
